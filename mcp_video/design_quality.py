...
class DesignQualityGuardrails:
    ...
    # Brand standards
    BRAND_COLORS = ['#CCFF00', '#5B2E91', '#7C3AED', '#6366F1']  # Electric Lime × Midnight Violet
            ...
            for issue in self.issues:
                if issue.fix_available and issue.auto_fix:
                    try:
                        ...
                        fixes_applied.append(f"FAILED {issue.category}: {str(e)}")
    ...
    def _check_layout(self, video_path: str):
        ...
        safe_margin_x = width * self.SAFE_AREA_MARGIN
        safe_margin_y = height * self.SAFE_AREA_MARGIN
    ...
    def _auto_fix_brightness(self, video_path: str, target: float = 128) -> str:
        ...
        cmd = [
            'ffmpeg', '-y', '-i', video_path,
            '-vf', f'eq=brightness=0.1:gamma=1.1',
            '-c:a', 'copy',
            output_path
        ]
