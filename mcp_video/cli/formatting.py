...
def _format_rescue_plan(result: Any) -> None:
    ...
    lines = [
        ...
        f"[bold green]Estimate:[/bold green] {estimate.get('seconds', 0)} seconds ({escape(str(estimate.get('confidence', 'unknown')))})",
...
def _format_rescue_render(result: Any) -> None:
    ...
    lines = [
        ...
        f"[bold green]Receipt:[/bold green] {escape(str(data.get('receipt_path') or 'rescue-receipt.json inside package'))}",
    ]
    _format_success_panel(lines, title="Rescue Render", border_style="green")


def _format_rescue_inspect(result: Any) -> None:
