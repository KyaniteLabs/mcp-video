...
def add_parsers(subparsers: argparse._SubParsersAction) -> None:
    ...
    rescue_plan.add_argument(
        "--output-dir", required=True, help="Directory for previews, plans, and rescue packages"
    )
    rescue_plan.add_argument(
        "--save-plan", default=None, help="Optional path to write the rescue plan as JSON"
    )
    rescue_plan.add_argument(
        "--policy",
    ...
    rescue_render.add_argument(
        "--save-receipt", default=None, help="Optional path to copy the render receipt as JSON"
    )
    rescue_render.add_argument(
        "--resume", default=None, help="Optional prior rescue receipt to resume"
    )
    rescue_render.add_argument(
        "--cancel-file", default=None, help="Cancel when this marker file exists"
    )
    rescue_render.add_argument(
        "--keep-intermediates",
        action="store_true",
    ...
    rescue_inspect.add_argument(
        "--receipt", required=True, help="Path to a rescue plan or render receipt"
    )
