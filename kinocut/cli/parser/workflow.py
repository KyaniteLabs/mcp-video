...
def add_parsers(subparsers: argparse._SubParsersAction) -> None:
    ...
    wf_plan.add_argument("--variant", default=None, help="Optional declared variant id to plan its effective steps")
    ...
    wf_render.add_argument("--resume", default=None, help="Optional path to a prior render receipt to resume from")
    wf_render.add_argument("--save-receipt", default=None, help="Optional path to write the workflow receipt as JSON")
    wf_render.add_argument(
        "--keep-intermediates",
        action="store_true",
        help="Retain @work intermediates even on success (recorded in the cleanup policy)",
    )
    wf_render.add_argument("--variant", default=None, help="Render a single declared variant by id")
    wf_render.add_argument(
        "--all-variants",
        action="store_true",
