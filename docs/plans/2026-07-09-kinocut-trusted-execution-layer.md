...
**status: APPROVED (Simon, 2026-07-10) — backlog filed: epic #85, issues #54–#84 + Track D #86–#94 (distribution/governance/users/security/decision-gates) + Track E #95–#107 (features/QoL; pillars: #105 Cutfile, #106 Video CI action, #107 conversational sessions), all gated `blocked:post-release` / milestone `kinocut-v2`; execution held until release + rename (#53)**
...
**Baseline:** `~/workspaces/mcp-video`, Forgejo source-of-truth. **Post-in-flight-release surface: 124 MCP tools + 103 CLI commands** (per current README / `docs/adr/0007-agent-workflow-engine.md`); master moved ~47 commits during planning and now ships the `video_workflow_*` engine. Exact tool/test counts are verified at cutover (P0.5), not hardcoded here. Codebase = "a guardrailed media RPC toolbox, not yet an agentic editor" (Codex audit, 2026-07-09), now with an ordered-job-spec workflow engine that resumes within a single spec (see Phase 1 reconciliation).
...
> **HARD GATE:** Nothing in this plan may start before (a) the in-flight release ships as `mcp-video`, and (b) the Kinocut rename (issue #53) executes. All post-release engineering issues carry a `blocked:post-release` label + `kinocut-v2` milestone so the ~30-min Factory auto-triage pipeline does not start them early. Removing the label is an explicit human act, phase by phase.
...
1. **Kernel over tool #125.** The next architectural move is a durable substrate, not another synchronous verb. The existing tool surface (124 at last count) becomes compatibility adapters, never breaking removals.
...
**Option B — Verbs/product-first (pure).** Ship the ~10 intent verbs + one-command repurposing on top of today's 124-tool surface; defer the kernel.
...
**Fate of the ~118 un-bridged tools.** Only the timeline-mutating repurposing-path subset (P1.6: trim, merge, subtitle burn-in, reframe/crop, silence-removal cuts) compiles into the kernel. **The remaining bulk of the 124-tool surface stays legacy path-in/path-out compatibility adapters *indefinitely*** — there is NO plan or phase to bridge all of them. They keep working unchanged; individual tools graduate into the kernel only on demand when a product path needs them. "124 tools" is a **depth claim**, not a migration backlog. This bounds the refactor and honors "never breaking removals."
...
- P2.2 **Steerable/trainable moment selection** (demand JTBD #1) — expose selection criteria (keywords, speaker, energy, retention heuristic) as parameters; "train on MY content" via example clips. New selection module under `mcp_video/kernel/` or `ai_engine/`.
...
- P3.0 **Promote the kernel to graph depth.** Land the pieces deferred from Phase 1 now that a consumer needs them: the full **Timeline IR** (stable node IDs, rational timebase, effect stacks, nested sequences) compiling to a **render DAG** (replaces the linear op-list render from P1.2 and the one-shot DTO role of `models.py:310` / `engine_timeline.py` for kernel paths); the **revision DAG** — `edit_project_fork`, `timeline_apply/diff/checkout`, branch/undo/compare-and-swap against `base_revision_id`; and the **full typed event plane** (`render.node.completed`, `render.segment.ready`, `review.*`). Findings can only anchor to `node_ids` once the IR exists, so this precedes P3.1. New: `mcp_video/kernel/timeline_ir.py`, `revision.py`; extend `events.py`, `render_jobs.py`.
**Work breakdown (Codex primitive #5):**
- P3.1 **Review API** — `review_policy_set`, `review_run`, `review_decide`, plus review events (`review.finding.created`, `review.patch.proposed/accepted`). New: `mcp_video/kernel/review.py`.
...
- P4.2 **OTIO in/out** — OpenTimelineIO import/export (the agent-legible EDL; interchange into DaVinci/Premiere/FCP). "Cheap to claim, unclaimed." New: `mcp_video/kernel/otio_io.py`.
...
|---|------|-------------------|------------|
| 1 | Kernel refactor destabilizes the existing suite or breaks the 124-tool surface / `video_workflow_*` engine | Med / High | Kernel is **additive** (`mcp_video/kernel/`), existing engine untouched; kernel WRAPS `video_workflow_render` (no rebuild); tools = compat adapters, never removals; TDD real-FFmpeg; green-suite gate at every phase exit. |
...
| 4 | Factory ~30-min auto-triage starts post-release work early or disrupts the in-flight release | Med / High | `blocked:post-release` label + `kinocut-v2` milestone on every issue; un-blocking is an explicit per-phase human act; in-flight release finishes as `mcp-video` before any P0 issue opens. |
...
- **Tool #120** — adding features instead of building the kernel (the meta-trap).
...
- **Follow-ups:** confirm in-flight release ship date (gates everything); write ADR-0008 (`docs/adr/0008-editing-kernel.md`) + CONTEXT.md noun disambiguation/amendment (P1.0) before any kernel code, including the `video_workflow_*` wrap decision; **separate Epoch sizing per kernel primitive** (project store, job store, render runner, CAS) before each issue opens; confirm the durable-job lifecycle (detached-subprocess-reconciled-on-startup) survives a real stdio relaunch test; decide allowed VLM providers + artifact-retention budget + remote-worker trust boundary (Codex flagged these as undefined policy); open `kinocut-v2` milestone + `blocked:post-release` label scheme in Forgejo.
...
- In-flight release ship date — the gate for all of Phase 0. (blocks start)
...
- Confirm exact post-release counts at cutover (P0.5): README/ADR say 124 MCP tools + 103 CLI commands — verify against `tests/test_public_surface.py` before quoting them anywhere.
...
**CRITIC VERDICT: APPROVE** (Rev 3; loop converged in 3 rounds: Architect SOUND-WITH-CHANGES → Rev 2 → Architect SOUND + Critic ITERATE → Rev 3 → Architect delta SOUND + Critic APPROVE). Plan remains **pending Simon's approval** — no execution until he approves AND the HARD GATE clears (release ships + rename executes).
