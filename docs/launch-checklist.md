...
video_watermark | video_export | video_edit | video_extract_audio
...
**Title:** Show HN: AgentCut – Video editing MCP server for AI agents (16 tools)
...
16 tools covering: trim, merge, text overlay, audio sync, resize, format conversion, speed change, thumbnails, previews, storyboards, subtitles, watermarks, export, full timeline edits, and audio extraction.
...
234 tests across the full testing pyramid. Pure Python, only depends on mcp + pydantic + ffmpeg.
...
It wraps FFmpeg into 16 structured tools. Works with Claude Code, Cursor, or any MCP client.
...
16 tools: trim, merge, add_text, add_audio, resize, convert, speed, thumbnail, preview, storyboard, subtitles, watermark, export, edit_timeline, extract_audio, info.

Also has a Python client and CLI. 234 tests. Apache 2.0.
...
AgentCut is an MCP server with 16 video editing tools. Add it to your Claude Code MCP settings, and you can just ask Claude to:
...
**Title:** AgentCut — open source video editing MCP server (16 tools, works with Claude/Cursor)
...
The architecture is: MCP server wrapping FFmpeg, with a Python client and CLI. 16 tools, 234 tests, Apache 2.0.
